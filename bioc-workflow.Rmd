...
analysis of such data. `r Biocpkg("pRoloc")` itself relies on `r
Biocpkg("MSnbase")` to manipulate and process quantitative proteomics
data. Many other packages are used by `r Biocpkg("pRoloc")` for
...
```{r normsum, eval = FALSE} hl <- normalise(hl, method = "sum") ```
...
replicates in the `r Biocexptpkg("pRolocdata")` package. Below, we
...
mean fraction intensities) and a `boxplot` of the column intensities.
In the two plots displayed below, we re-order the fractions to pair
corresponding fractions in the two replicates (rather than ordering
the fractions by replicate).
...
```{r plot2Drep, fig.width = 12} par(mfrow = c(1, 2)) plot2D(hl[,
hl$Replicate == 1], main = "Replicate 1") plot2D(hl[, hl$Replicate ==
2], main = "Replicate 2", mirrorX = TRUE) ```
...
In the code chunk below we demonstrate how to run the `phenoDisco`
...
information we refer users to the `r Biocpkg("pRolocGUI")` tutorial 
...
the nearest neighbour calculations for each data source. 

```{r knnoptfortl, eval=FALSE}
## get best k's
