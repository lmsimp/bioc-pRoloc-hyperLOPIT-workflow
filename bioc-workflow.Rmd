...
Visualisation and data exploration is an important aspect of data analyses allowing one to shed light on data structure and patterns of interest. Using the `r Biocpkg("pRolocGUI")` package we can interactively visualise, explore and interrogate quantitative spatial proteomics data. The `r Biocpkg("pRolocGUI")` package is currently under active development and it relies on the `shiny` framework for reactivity and interactivity. The package currently distributes 3 different GUI's ("main" (default), "compare" or "compare") which are wrapped and launched by the `pRolocVis` function. 
...
![A screen shot of clickable interface and zoomable PCA plot of the main app in the `r Biocpkg("pRolocGUI")` package.](./Figures/mainapp.png)
