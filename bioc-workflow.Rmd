...
* Need to say that can be label-free or any other type of
  quantitation, and, possibly, that features can be a peptide level.

* I would like to show code for the below - maybe move to later
  sections. See *Data processing* section.
...
The output `params` is an object of class `GenRegRes`; a regularisation/optimisation framework container for the optimisation results.


associated macro F1 scores. A high macro F1 score
indicates that the marker proteins in the test dataset are consistently correctly assigned by
17
Spatial proteomics data analysis
the algorithm. Often more than one parameter or set of parameters gives rise to the best
generalisation accuracy. As such it is always important to investigate the model parameters
and critically assess the best choice. The best choice may not be as simple as the parameter
set that gives rise to the highest macro F1 score and one must be careful to avoid overtting
and to choose parameters wisely.
