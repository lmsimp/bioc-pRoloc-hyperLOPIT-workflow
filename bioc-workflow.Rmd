...
`r Biocpkg("pRoloc")`, which offers a set of dedicated functions for
the analysis of such data. `r Biocpkg("pRoloc")` itself relies on 
`r Biocpkg("MSnbase")` to manipulate and process quantitative
proteomics data. Many other packages are used by `r Biocpkg("pRoloc")`
...
Installation of Bioconductor packages are documented in detail on the
...
As discussed in the manuscript [@hyper] it has been shown that
...
the `r Biocexptpkg("pRolocdata")` package for Christoforou's
