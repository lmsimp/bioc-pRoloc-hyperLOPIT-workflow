...
annotations and sequences, and protein-protein interactions among others, which represent a rich and vast source of complementary information. The integration of data between different technologies is one of the biggest challenges in computational biology to date. A paradigm in machine learning, known as transfer learning (TL) is ideally suited to this task. The `r Biocpkg("pRoloc")` package provides functionality to do such analyses.

In TL one typically has a primary task that they wish to solve, and some complementary (often heterogeneous) auxiliary information that is related to the primary learning objective, that can be used to help solve the primary goal. For example, here our primary task is to assign proteins to their sub-cellular niche with high generalisation accuracy from data collected from quantitative MS-based experiments. One useful source of information that may help us solve our task is Gene Ontology information, such as the cellular compartment namespace, for every protein found in the primary experiment. We have shown that using transfer learning in the context of spatial proteomics results in the assignment of proteins to sub-cellular niches with higher generalisation accuracy than using standard supervised machine learning [@Breckels:2016]. 
