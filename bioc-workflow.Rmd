...
Sample meta-data distributed with
the `r Biocexptpkg("pRolocdata")` and as above with the quantitation
data file is located in the `extdata` in the `r Biocexptpkg("pRolocdata")` 
...
it into R using `read.csv` and append the meta-data to the `pData` slot. 
We add information on the gradient fractions used and the associated 
...
From examining the literature and the Gene Ontology we find that
phenotype 1 is enriched in proteins that localise to the nucleus
(non-chromatin associated proteins), phenotype 2 is enriched in
chromatin associated proteins, phenotype 3 contains many cytosolic
proteins and phenotypes 4 and 5 contain lysosmal and endosomal
proteins. From curating these phenotypes we obtain new marker proteins
that can be used as input for supervised classification.
