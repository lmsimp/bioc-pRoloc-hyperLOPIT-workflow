...
feature names. See `?addMarkers` for more details.
...
paradigm known as transfer learning (TL). Transfer learning may be
particuarly useful when sub-cellular niches are not equally
well-resolved in a particular experiment, or if we have overlapping
clusters in the data.

The integration of data between different technologies is one of the
biggest challenges in computational biology to date and the
\Biocpkg{pRoloc} package provides functionality to do such analyses.
We recently developed two transfer learning algorithms using a $k$-NN
and SVM framework and applied them to the task of protein localisation
prediction \cite{Breckels:2016}. In this section we will begin with
explaining the concept of transfer learning and then show how to apply
this in the frame of spatial proteomics and protein localisation
prediction.
...
these types of experiments, however, 
Transfer learning is particularly useful for
classes that are not as well separated.
