...
protein groups that were not acquired in both replicates (Figure
\ref{fig:namap}).
...
accession numbers and the dataset entries are labelled by Uniprot entry
names, one will have to convert and match the proteins according to
