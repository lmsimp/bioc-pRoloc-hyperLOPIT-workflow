...
\begin{figure}[!ht]
  \centering
  \includegraphics[width=.45\textwidth]{./Figures/msnset.png}
...
\begin{figure}[!ht]
  \centering
<<qcplot, message=FALSE, warning=FALSE, out.width="\\textwidth">>=
library("pRoloc")
...
\begin{figure}[!ht]
  \centering
<<plotmarkers, out.width="\\textwidth">>=
par(mfrow = c(1, 2))
...
\begin{figure}[!ht]
  \centering
<<visualiseOptHide, echo=FALSE, out.width="\\textwidth">>=
library('gridExtra')
...
\begin{figure}[!ht]
  \centering
<<plottl, out.width="\\textwidth">>=
plot(tlopt)
@  
  \caption{Visualisation of the tTransfer learning parameter optimisation procedure.}
  \label{fig:plottl}
\end{figure}
