...
\hl{relevent} for any quantitative spatial proteomics data, irrespective of
...
There are two aspects related to data normalisation that are \hl{relevent}
...
profiles into \hl{the} same intensity interval (typically 0 and 1) by, for
...
{\ttfamily\noindent\color{warningcolor}{\#\# Warning: 'combine' is deprecated.\\\#\# Use 'gtable\_combine' instead.\\\#\# See help("{}Deprecated"{})}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in `[[<-.data.frame`(`*tmp*`, i, value = structure(list(t = numeric(0), : replacement has 0 rows, data has 10}}\begin{alltt}
\hlstd{combined}
\end{alltt}
\begin{verbatim}
## MSnSet (storageMode: lockedEnvironment)
...
## Annotation:  
## - - - Processing information - - -
## Combined [5489,10] and [6268,10] MSnSets Tue Apr 10 11:36:36 2018 
## Subset [6725,20][5032,20] Tue Apr 10 11:36:36 2018 
## Removed features with more than 0 NAs: Tue Apr 10 11:36:36 2018 
## Dropped featureData's levels Tue Apr 10 11:36:36 2018 
## Subset [5032,20][5032,20] Tue Apr 10 14:00:41 2018 
## Removed features with more than 0 NAs: Tue Apr 10 14:00:41 2018 
## Dropped featureData's levels Tue Apr 10 14:00:41 2018 
## Subset [5032,20][5032,20] Tue Apr 10 16:11:22 2018 
## Removed features with more than 0 NAs: Tue Apr 10 16:11:22 2018 
## Dropped featureData's levels Tue Apr 10 16:11:22 2018 
## Subset [5032,20][5032,20] Tue Apr 10 16:17:52 2018 
## Removed features with more than 0 NAs: Tue Apr 10 16:17:52 2018 
## Dropped featureData's levels Tue Apr 10 16:17:52 2018 
## Subset [5032,20][5032,20] Tue Apr 10 16:25:51 2018 
## Removed features with more than 0 NAs: Tue Apr 10 16:25:51 2018 
## Dropped featureData's levels Tue Apr 10 16:25:51 2018 
## Subset [5032,20][5032,20] Tue Apr 10 16:29:26 2018 
## Removed features with more than 0 NAs: Tue Apr 10 16:29:26 2018 
## Dropped featureData's levels Tue Apr 10 16:29:27 2018 
## Subset [5032,20][5032,20] Tue Apr 10 16:35:47 2018 
## Removed features with more than 0 NAs: Tue Apr 10 16:35:47 2018 
## Dropped featureData's levels Tue Apr 10 16:35:47 2018 
## Subset [5032,20][5032,20] Tue Apr 10 16:47:20 2018 
## Removed features with more than 0 NAs: Tue Apr 10 16:47:20 2018 
## Dropped featureData's levels Tue Apr 10 16:47:20 2018 
...
sub-cellular resolution. \st{Missing data can be imputated} \hl{We can impute missing data} using
...
## Removed features with more than 0 NAs: Tue Apr 10 16:47:20 2018 
## Dropped featureData's levels Tue Apr 10 16:47:20 2018 
## Subset [5032,20][5032,20] Tue Apr 10 16:52:54 2018 
## Removed features with more than 0 NAs: Tue Apr 10 16:52:54 2018 
## Dropped featureData's levels Tue Apr 10 16:52:54 2018 
##  MSnbase version: 2.5.9
\end{verbatim}
\end{kframe}
\end{knitrout}
...
pre-defined colours. \hl{As previously mentioned, PCA transforms the
...
\hl{This is further highlighted if we plot the
profiles of these two clusters using the \texttt{plotDist} function}
(Figure \ref{fig:plotDist2}). \hl{The \texttt{plotDist} function is
...
figure} \ref{fig:plotDist2}, \hl{we plot the profile of the mitochondrial
...
visualise these}. For example, if we wanted to highlight \st{a} 
...
##  Created on Tue Apr 10 16:52:56 2018 
##  Description:
...
do\st{es} not provide an adequate, goal-driven assessment of different
...
The more \hl{relevent} comparison unit is not a single channel, but rather
...
\hl{The above analysis is comptationally intensive and best parallelised over
multiple workers. This phenoDisco analysis took ~24 hours to complete
when parallelised over 40 workers. As such, to avoid executing this
time consuming analysis users can access the above results which are
...
## Annotation:  
## - - - Processing information - - -
## Added markers from  'mrk' marker vector. Tue Apr 10 16:52:55 2018 
## Added markers from  'pdres' marker vector. Tue Apr 10 16:52:56 2018 
##  MSnbase version: 2.5.10
\end{verbatim}
\begin{alltt}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{pRolocVis}\hlstd{(hl,} \hlkwc{fcol} \hlstd{=} \hlstr{"pd"}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\href{http://bioconductor.org/packages/release/bioc/vignettes/pRoloc/inst/doc/pRoloc-ml.pdf}{\Biocpkg{pRoloc}
  machine learning techniques vignette}.  We find the general \hl{tendency}
...
\hl{In the code chunk below we then pass these weights 
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{svmf} \hlkwb{<-} \hlkwd{dir}\hlstd{(extdatadir,} \hlkwc{full.names} \hlstd{=} \hlnum{TRUE}\hlstd{,}
              \hlkwc{pattern} \hlstd{=} \hlstr{"bpw-svmopt.rds"}\hlstd{)}
\hlstd{params} \hlkwb{<-} \hlkwd{readRDS}\hlstd{(svmf)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\color{warningcolor}{\#\# Warning in seq\_len(trpr): first element used of 'length.out' argument}}\end{kframe}
\end{knitrout}
...
per organelle by extracting the median or 3rd \hl{quartile} score per
...
\begin{verbatim}
##            40S Ribosome            60S Ribosome      Actin cytoskeleton 
##               0.4323379               0.3037630               0.3822328 
##                 Cytosol   Endoplasmic reticulum                Endosome 
##               0.6930642               0.6014878               0.4426545 
##    Extracellular matrix                Lysosome           Mitochondrion 
##               0.4224179               0.5978957               0.9497788 
##     Nucleus - Chromatin Nucleus - Non-chromatin              Peroxisome 
##               0.7950203               0.7117761               0.3147679 
...
\hl{(We also provide the pre-computed \texttt{gocc} object for users
...
parameter \hl{search} and test weights $\theta = {0, 1/3, 2/3, 1}$ for each
...
In the example below we \hl{first} remove these 4 classes from the marker set
\hl{and create a new marker set called tlmarkers. Then we}
...
\hl{To save time for users, the results of the following optimisation 
...
\st{To remove the 4 classes and create a new column of markers in the
...
From examining the parameter \hl{search} plots as described in section
...
\texttt{save} function, and later reload the object using \st{save}
...
\section*{Session information and getting help}
...
##  [1] gridExtra_2.3        pRolocdata_1.17.3    pRoloc_1.19.3       
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{## Specifying the path to the PD results}
\hlstd{f0} \hlkwb{<-} \hlkwd{dir}\hlstd{(extdatadir,} \hlkwc{full.names} \hlstd{=} \hlnum{TRUE}\hlstd{,}
          \hlkwc{pattern} \hlstd{=} \hlstr{"bpw-pdres.rds"}\hlstd{)}
\hlcom{## Reading the PD results}
\hlstd{pdres} \hlkwb{<-} \hlkwd{readRDS}\hlstd{(f0)}
\hlcom{## Adding the PD results to the hl MSnSet}
\hlstd{hl} \hlkwb{<-} \hlkwd{addMarkers}\hlstd{(hl, pdres,} \hlkwc{mcol} \hlstd{=} \hlstr{"pd"}\hlstd{,} \hlkwc{verbose} \hlstd{=} \hlnum{FALSE}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{## Specifying the path to the SVM optimisation results}
\hlstd{svmf} \hlkwb{<-} \hlkwd{dir}\hlstd{(extdatadir,} \hlkwc{full.names} \hlstd{=} \hlnum{TRUE}\hlstd{,}
              \hlkwc{pattern} \hlstd{=} \hlstr{"bpw-svmopt.rds"}\hlstd{)}
\hlcom{## Reading the results}
\hlstd{params} \hlkwb{<-} \hlkwd{readRDS}\hlstd{(svmf)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
To load the GO CC dataset 

\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{gofile} \hlkwb{<-} \hlkwd{dir}\hlstd{(extdatadir,} \hlkwc{full.names} \hlstd{=} \hlnum{TRUE}\hlstd{,}
              \hlkwc{pattern} \hlstd{=} \hlstr{"bpw-gocc.rds"}\hlstd{)}
\hlstd{gocc} \hlkwb{<-} \hlkwd{readRDS}\hlstd{(gofile)}
\end{alltt}
\end{kframe}
\end{knitrout}

To load the results of the transfer learning

\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{tlfile} \hlkwb{<-} \hlkwd{dir}\hlstd{(extdatadir,} \hlkwc{full.names} \hlstd{=} \hlnum{TRUE}\hlstd{,}
              \hlkwc{pattern} \hlstd{=} \hlstr{"bpw-tlopt.rds"}\hlstd{)}
\hlstd{tlopt} \hlkwb{<-} \hlkwd{readRDS}\hlstd{(tlfile)}
\end{alltt}
\end{kframe}
\end{knitrout}
