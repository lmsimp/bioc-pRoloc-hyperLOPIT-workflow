...
<<loadPDres_apx, eval=FALSE>>=
## Specifying the path to the PD results
f0 <- dir(extdatadir, full.names = TRUE,
          pattern = "bpw-pdres.rds")
## Reading the PD results
pdres <- readRDS(f0)
## Adding the PD results to the hl MSnSet
hl <- addMarkers(hl, pdres, mcol = "pd", verbose = FALSE)
@
...
<<loadpdRes_apx, eval=FALSE>>=
## Specifying the path to the SVM optimisation results
svmf <- dir(extdatadir, full.names = TRUE,
              pattern = "bpw-svmopt.rds") 
## Reading the results
params <- readRDS(svmf)
