...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in `[.data.frame`(pData(object), , .frac): object '.frac' not found}}\begin{alltt}
\hlkwd{title}\hlstd{(}\hlkwc{main} \hlstd{=} \hlstr{"Marker occupancy profiles along the gradient"}\hlstd{)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in title(main = "{}Marker occupancy profiles along the gradient"{}): plot.new has not been called yet}}\begin{alltt}
...
It is also worthly of note that it is possible to search for a
