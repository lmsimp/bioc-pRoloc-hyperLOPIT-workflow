...
embryonic stem cells experiment \cite{hyper}. Additional annotated and
pre-formatted datasets from various species are readily available in
the \Biocexptpkg{pRolocdata} package.
...
\cite{hyper} as it has been shown that combining replicates across
from different gradients can increase spatial resolution
...
To make use of the full functionality of the \Biocpkg{pRoloc}
software one needs to import their data into R and prepare them as an
\texttt{MSnSet}. The \texttt{MSnSet} is a dedicated data structure for the efficient
manipulation and processing of mass spectrometry and proteomics data
in R. Figure 1 illustrates a simplified view of the \texttt{MSnSet}
structure; there exists 3 key sub-parts (termed slots) to such a data
object: (1) the \texttt{exprs} slot for storing the quantitation data, (2)
the \texttt{fData} slot for storing the feature meta-data, and finally (3)
the \texttt{pData} slot for storing the sample meta-data.
...
function \texttt{readMSnSet2} in \Biocpkg{MSnbase}. The function
takes a single spreadsheet as input and extracts the columns
containing the quantitation data, as identified by the argument
\texttt{ecol}, to create the expression data, while the other columns
in the spreadsheet are appended to the feature meta-data slot.  By
example, in the code chunk below we read in the \texttt{csv}
spreadsheet containing the quantitation data from the intersect of
replicates 1 and 2 of the mouse map \cite{hyper}, using the
\texttt{readMSnSet2} function. The data is as available online with
the manuscript (see tab 2 of the \texttt{xlsx} supplementary data set
1 in \cite{hyper}, which should be exported as a text-based
spreadsheet). It is also available as a \texttt{csv} in the
Bioconductor \Biocexptpkg{pRolocdata} data package.
...
\texttt{gzip}, extension), and will be decompressed on-the-fly when read into
R later on.
...
*Combining MSnSet instances* section of the \Biocpkg{MSnbase}
...
\Biocpkg{MSnbase}'s \texttt{impute} function. The method underlying the
imputation method is then determined by a \texttt{methods} parameter. In our
...
replicated experiments. This is done with the \texttt{filterNA} function that
...
## - - - Processing information - - -
## Combined [6725,20] and [6268,10] MSnSets Thu Sep 29 20:32:05 2016 
## Subset [6725,20][5032,20] Thu Sep 29 21:02:54 2016 
## Removed features with more than 0 NAs: Thu Sep 29 21:02:54 2016 
## Dropped featureData's levels Thu Sep 29 21:02:54 2016 
##  MSnbase version: 1.21.7
\end{verbatim}
\end{kframe}
\end{knitrout}
...
the \texttt{plot2D} function, which can be parametrised with the
\texttt{method} argument.  Advantages, disadvantages? See
\href{https://gist.github.com/mikelove/74bbf5c41010ae1dc94281cface90d32}
...
classifier will be trained and generated. \Biocpkg{pRoloc}
...
the mouse markers stored in \Biocpkg{pRoloc} (\texttt{mmus} markers) 
...
\hlkwd{addLegend}\hlstd{(hl,} \hlkwc{cex} \hlstd{=} \hlnum{.7}\hlstd{)}
...
\end{alltt}
\end{kframe}
\includegraphics[width=.65\textwidth]{figure/plotDist-1} 
...
(also mentioned later), although these are only available in the
development version.
...
% <<repl1, eval = TRUE, echo=FALSE, fig.width = 12>>=
...
interest. Using the \Biocpkg{pRolocGUI} package we can
interactively visualise, explore and interrogate quantitative spatial
proteomics data. The \Biocpkg{pRolocGUI} package is currently
...
\hlkwd{data}\hlstd{(hyperLOPIT2015ms3r1)}
\hlkwd{data}\hlstd{(hyperLOPIT2015ms3r2)}
\hlstd{mydata} \hlkwb{<-} \hlkwd{MSnSetList}\hlstd{(}\hlkwd{list}\hlstd{(hyperLOPIT2015ms3r1, hyperLOPIT2015ms3r2))}
\hlkwd{pRolocVis}\hlstd{(mydata,} \hlkwc{app} \hlstd{=} \hlstr{"compare"}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hlstd{hl} \hlkwb{<-} \hlkwd{phenoDisco}\hlstd{(hl,} \hlkwc{fcol} \hlstd{=} \hlstr{"phenoDisco.Input"}\hlstd{,}
                 \hlkwc{times} \hlstd{=} \hlnum{200}\hlstd{,}
                 \hlkwc{GS} \hlstd{=} \hlnum{60}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
The ouput of running the \texttt{phenoDisco} algorithm is a \texttt{MSnSet}
...
## Annotation:  
## - - - Processing information - - -
## Added markers from  'mrk' marker vector. Thu Sep 29 21:02:55 2016 
## Added markers from  'pdres' marker vector. Thu Sep 29 21:02:56 2016 
##  MSnbase version: 1.99.2
\end{verbatim}
\begin{alltt}
...
\Biocpkg{pRoloc}, algorithmic performance is estimated using
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{hl} \hlkwb{<-} \hlkwd{svmClassification}\hlstd{(hl, params,}
                        \hlkwc{class.weights} \hlstd{= w,}
                        \hlkwc{fcol} \hlstd{=} \hlstr{"markers"}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\subsubsection*{Thresholding}\label{sec:thresholding}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{ts} \hlkwb{<-} \hlkwd{orgQuants}\hlstd{(hl,}
                \hlkwc{fcol} \hlstd{=} \hlstr{"svm"}\hlstd{,}
                \hlkwc{scol} \hlstd{=} \hlstr{"svm.scores"}\hlstd{,}
                \hlkwc{mcol} \hlstd{=} \hlstr{"markers"}\hlstd{,}
                \hlkwc{t} \hlstd{=} \hlnum{.5}\hlstd{)}
\end{alltt}
\begin{verbatim}
##            40S Ribosome            60S Ribosome      Actin cytoskeleton 
...
\end{verbatim}
\begin{alltt}
\hlstd{hl} \hlkwb{<-} \hlkwd{getPredictions}\hlstd{(hl,}
                     \hlkwc{fcol} \hlstd{=} \hlstr{"svm"}\hlstd{,}
                     \hlkwc{scol} \hlstd{=} \hlstr{"svm.scores"}\hlstd{,}
                     \hlkwc{mcol} \hlstd{=} \hlstr{"markers"}\hlstd{,}
                     \hlkwc{t} \hlstd{= ts)}
...
\hlkwd{library}\hlstd{(}\hlstr{"pRolocGUI"}\hlstd{)}
\hlkwd{pRolocVis}\hlstd{(hl,}
          \hlkwc{app} \hlstd{=} \hlstr{"classify"}\hlstd{,}
          \hlkwc{fcol} \hlstd{=} \hlstr{"svm"}\hlstd{,}
                \hlkwc{scol} \hlstd{=} \hlstr{"svm.scores"}\hlstd{,}
          \hlkwc{mcol} \hlstd{=} \hlstr{"markers"}\hlstd{)}
\end{alltt}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{par} \hlkwb{<-} \hlkwd{setAnnotationParams}\hlstd{(}\hlkwc{inputs} \hlstd{=} \hlkwd{c}\hlstd{(}\hlstr{"Mus musculus"}\hlstd{,}
                                      \hlstr{"UniProt/Swissprot"}\hlstd{))}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{gocc} \hlkwb{<-} \hlkwd{makeGoSet}\hlstd{(hl,} \hlkwc{params} \hlstd{= par,}
                  \hlkwc{namespace} \hlstd{=} \hlstr{"cellular_component"}\hlstd{,}
                  \hlkwc{evidence} \hlstd{=} \hlkwa{NULL}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hlkwd{plot}\hlstd{(kpopt)}
\hlkwd{plot}\hlstd{(kaopt)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hlstd{tlopt} \hlkwb{<-} \hlkwd{knntlOptimisation}\hlstd{(hl, gocc,}
                           \hlkwc{fcol} \hlstd{=} \hlstr{"tlmarkers"}\hlstd{,}
                           \hlkwc{length.out} \hlstd{=} \hlnum{4}\hlstd{,}
                           \hlkwc{times} \hlstd{=} \hlnum{50}\hlstd{,}
...
\hlstd{hl} \hlkwb{<-} \hlkwd{knntlClassification}\hlstd{(hl, gocc,} \hlkwc{bestTheta} \hlstd{= bestpar,}
                          \hlkwc{k} \hlstd{=} \hlkwd{c}\hlstd{(}\hlnum{3}\hlstd{,} \hlnum{3}\hlstd{))}
\end{alltt}
\end{kframe}
\end{knitrout}
...
## [88] tibble_1.2            msdata_0.12.1         car_2.1-3            
...
\textbf{TODO} In order to give appropriate credit to each author of an
article, the individual contributions of each author to the manuscript
should be detailed in this section. We recommend using author initials
and then stating briefly how they contributed.
